...
function parseSubredditAllowlist(raw: string): string[] {
  return raw
    .split(',')
    .map((entry) => entry.trim().toLowerCase().replace(/^r\//, ''))
    .filter(Boolean);
}
...
export const config = {
  ...
  subredditAllowlist: parseSubredditAllowlist(process.env.SUBREDDIT_ALLOWLIST || '')
};
